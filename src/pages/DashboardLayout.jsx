...
function NavButton({ item, active, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        background: active
          ...
          : "transparent",
        color: highlighted ? textOn : textSub,
    ...
    >
      ...
      >
        <item.Icon className="w-4 h-4" style={{ color: highlighted ? textOn : textSub }} />
      </div>
      <span className="flex-1 truncate">{item.label}</span>
      {active && (
        <ChevronRightIcon className="w-3.5 h-3.5 shrink-0" style={{ color: "rgba(255,255,255,0.4)" }} />
      )}
    </button>
  )
}
...
export default function DashboardLayout() {
  ...
  const SidebarContent = () => (
    <>
      {/* ── UPDATED: BIGGER LOGO SECTION ── */}
      <div
        className="flex items-center gap-3 px-5 py-4 shrink-0"
        style={{ borderBottom: `1px solid ${border}` }}
      >
        {/* Removed the background/border box and made it w-12 h-12 with 100% image size */}
        <div className="w-12 h-12 flex items-center justify-center shrink-0 drop-shadow-md">
          <img src={lorriLogo} alt="LoRRI Logo" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
        </div>
        ...
        <button
          className="ml-auto lg:hidden w-7 h-7 flex items-center justify-center rounded-lg"
      ...
      <div className="px-4 py-4 shrink-0" style={{ borderTop: `1px solid ${border}` }}>
        <div className="flex items-center gap-3 mb-3 px-1">
          <div
            ...
            style={{ background: colors.gradientAccent, color: colors.textWhite }}
          >
            JD
          </div>
          <div className="min-w-0">
            <div style={{ color: textOn, fontSize: typography.sm, fontWeight: typography.medium }}>John Doe</div>
            <div style={{ color: textSub, fontSize: "10px" }} className="truncate">john@company.com</div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              style={{ background: "rgba(255,255,255,0.08)", border: `1px solid ${border}`, color: textSub }}
            ...
            <div className="flex items-center gap-2">
              <span className="hidden sm:inline" style={{ color: textSub, fontSize: typography.sm }}>LoRRI.ai</span>
