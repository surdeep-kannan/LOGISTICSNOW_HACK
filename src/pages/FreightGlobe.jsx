...
export default function FreightGlobe({ hubs = [], lanes = [], activeHub, setActiveHub, filterMode = 'all', simplified = false }) {
    ...
    const onGlobeReady = useCallback(() => {
        ...
        ctrl.autoRotate = false; // Disabled as per user request
    ...
    useEffect(() => {
        ...
        // Only set region focus if filterMode changed AND no activeHub
        const regions = {
        ...
        // Debounce or only trigger on explicit filterMode update to prevent conflict
        g.pointOfView(target, 1200);

    }, [activeHub, filterMode, ready]); // Removed hubs from deps to prevent re-flying on data updates
    ...
    const arcLabel = arc => `
    <div style="background:rgba(69,61,154,0.97);border:1px solid rgba(100,160,255,0.25);border-radius:10px;padding:10px 14px;color:#fff;font-family:sans-serif;min-width:170px;box-shadow:0 8px 32px rgba(0,0,0,0.8)">
    ...
    <div style="background:rgba(69,61,154,0.97);border:1px solid rgba(100,160,255,0.2);border-radius:10px;padding:10px 14px;color:#fff;font-family:sans-serif;min-width:155px;box-shadow:0 8px 32px rgba(0,0,0,0.8)">
